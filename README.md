...
The CMake build does not require Autoconf, Automake, Libtool, a Unix shell,
or GNU make. Generated headers and preprocessed assembly are written only to
the selected binary directory. A typical out-of-source build is:
...
On Windows, run these commands in the appropriate Visual Studio developer
command prompt. Shared libraries are built by default; pass
``-DBUILD_SHARED_LIBS=OFF`` for a static library.

Installed CMake consumers can use:

    find_package(libffi 3.8 CONFIG REQUIRED)
    target_link_libraries(application PRIVATE libffi::ffi)
